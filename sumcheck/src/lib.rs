...
fn generate_partial_proof <F: PrimeField, H: HashWrapper, T: TranscriptTrait<H>> (poly: &Composite<F>, transcript: &mut T, round_polys: &mut Vec<Vec<F>>) -> F {
    ...
    for i in 0..rounds {
        ...
        let _sum = round_poly[0] + round_poly[1];
        if i == 0 {sum = _sum;}
        let mut data = vec![_sum];
...
fn verify_partial_proof<F: PrimeField, H: HashWrapper, T: TranscriptTrait<H>> (sum: F, polys: &Vec<Vec<F>>, transcript: &mut T) -> (F , Vec<F>) {
...
fn add_data_to_transcript <F: PrimeField, H: HashWrapper, T: TranscriptTrait<H>> (data: &Vec<F>, transcript: &mut T) -> F {
    let mut bytes = vec![];
    data.iter().for_each(|x| {
        bytes.extend(x.into_bigint().to_bytes_be())
    });
    let challenge = F::from_be_bytes_mod_order(&transcript.append(&bytes));
    return challenge;
}
...
mod test {
    ...
    fn test_generate_partial_proof() {
        ...
        let hasher = Keccak256::new();
        let mut transcript = Transcript::new(hasher);
        let initial_sum = generate_partial_proof(&composite, &mut transcript, &mut round_polys);
